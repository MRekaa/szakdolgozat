...
\begin{document}
	...
	\begin{itemize}
		...
		\item \textbf{Oktatási célokra alkalmas:} A Python széles körben használt oktatási nyelvként, amely ezáltal ideális oktatási célú alkalmazás fejlesztéséhez
	\end{itemize}
	...
	A Structured Query Language (röviden SQL\cite{SQL}) az IBM-nél indult fejlesztésként jelent meg az 1970-es években, és relációs adatbázisok kezelésére szolgál. Főbb feladatai közé tartozik az adatok lekérdezése, módosítása és törlése, valamint adatbázisok létrehozása és kezelése.
	...
	A GitHub\cite{GitHub} egy webalapú platform, mely a Git\cite{Git} verziókezelő rendszerre épül. Fontos szerepet játszik a kód online tárolásában és elérhetőségében. Lehetőséget ad több fejlesztő együttműködésére és a kód verziókezelésére.
	
	A fejlesztést lokálisan végeztem, a GitHubot pedig elsősorban a forráskód publikálására és a beadáskor történő elérhetővé tételére használtam
	...
	A phpMyAdmin böngészőben futtatható kezelőfelület, amely MySQL-adatbázisok adminisztrációját támogatja.
	...
	A csomag használata lehetővé tette, hogy az alkalmazás adatvezérelt módon működjön, így az anyagok, eszközök és reakciók hozzáadása egyszerűen, az adatbázis bővítésével megvalósítható, a programkód  módosítása nélkül.
	...
	A játék lehetőséget biztosít különböző anyagok és eszközök kombinálására, amelyek segítségével új vegyületek hozhatók létre. Ez a folyamat hozzájárul a tanulók memóriájának fejlesztéséhez, valamint az összefüggések könnyebb megértéséhez. Az anyagok vizuális megjelenítése támogathatja a fogalmak könnyebb felismerését és a reakciók	közötti kapcsolatok megértését.
	
	\section{Gamifikáció}
	
	A rendszertervezése során kiemelt szempontom volt a gamifikáció\cite{Gamifikacio} Ez játékmechanikai elemek oktatási környezetbe történő beépítését jelenti, amelyek támogatják a felhasználói motiváció növelését és a tanulás aktívabbá tételét. A gamifikáció\cite{Gamifikacio2} oktatási szerepe főként a motiváció erősítésében jelenik meg, különösen fontos a természettudományos, elvont fogalmak tanulása során, mint amilyenek a kémiai reakciók is.
	
	A program megközelítése lehetővé teszi az önálló felfedezésen alapuló tanulást, ahol a tanulók nem egy előre meghatározott, lineáris útvonalon haladnak, hanem szabadon próbálhatják ki az anyagokat és eszközöket. Saját tapasztalati úton fedezhetik fel a kémiai reakciókat, ami elősegíti a megértést és a hosszabb távú tudást. Minden interakcióra azonnali vizuális megjelenés történik, ezzel támogatva a tanulás hatékonyságát.
	
	Fontos volt az is, hogy a hibázás kockázatmentes legyen, ne legyen büntető rendszer, hátráltató tényező. Így egy biztonságos kísérletezési környezetet teremtve, ahol a tanulók aktív tapasztalatszerzésen keresztül bővíthetik tudásukat. A beépített keresőrendszer segítséget nyújt  elakadás során, ezáltal nem érheti kudarc a felhasználót tanulás során.
	
	A vizuális megjelenés, tovább erősíti a tanulási akarást és élményt. A tanulási folyamatot többféle visszajelzés támogatja: a felhasználó egyszerre látja az elemeket,
	olvassa a reakció eredményét, és interakcióba lép a rendszerrel. Ezek biztosítják, hogy az program ne csupán egy eszköz legyen, hanem egy motiváló, élményalapú tanulási környezet.
	...
	\begin{itemize}
		\item  A reactions tábla tárolja a reakciókat az item1, item2, result, equation és type mezők segítségével.
		\item  Az elérhető anyagok listája (name).
		\item  Az elérhető eszközök listája (name).
	\end{itemize}
	
	\section{Fő osztályok:}
	\begin{itemize}
	...
	\begin{itemize}
		...
		\item Az itemek pozíciójának frissítése drag-and-drop műveletek során
		\item Az itemek megjelenítésének koordinálása a Pygame-gal
	...
	\begin{itemize}
		...
		\item \textbf{toolsH.py}: Az elérhető eszközöket jeleníti meg hasonló módon.
	\end{itemize}
	...
	A Trash osztály az objektumok törléséhez szükséges felhasználói felületi komponenst valósítja meg. Alapvető feladatai közé tartozik a törlési zóna megjelenítése és pozícionálása az ablak egy rögzített helyén, a jobb felső sarokban.
	Az egérmozgás figyelése és annak detektálása, hogy mely objektumok vannak a törlési zónában
	Vizuális visszajelzést ad, ha egy item a törlési zónába kerül, a kuka sprite-ja megváltozik
	...
	Az objektumokat definiáló osztályok. A \ref{fig:tools}. ábra a ToolsH osztály egyszerűsített felépítését mutatja be
	
	\lstinputlisting[style=pythonstyle,caption=Tool osztály,label=fig:tools]{tool.py}
	
	\subsubsection{Settings modul (settings.py)}
	
	Az alkalmazás beállításait és konstansait tárolja:
	\begin{itemize}
	...
	\begin{enumerate}
		...
		\item Minden képkockában a rendszer feldolgozza a felhasználói eseményeke: egér eseményeket, mozgásokat és drag-and-drop operációkat.
	...
	\begin{itemize}
		\item \textbf{Ablak és megjelenítés:} A Pygame kezeli az alkalmazás ablakát, a felbontást és az átméretezést. A \texttt{pygame.display} modul kezeli az ablak létrehozását és a képernyő frissítését.
	...
	\begin{itemize}
		\item \textbf{mysql-connector-python csomag:} Ez a csomag a MySQL szerver és a Python alkalmazás közötti kommunikációt kezeli. Az \texttt{dataBase.py} modul az alábbi műveleteket végzi:
		\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{ItemOnBoard → Reaction:} Az ItemOnBoard modul nyomon követi a játékmezőn lévő objektumokat, majd a lehetséges kombinációkról információt ad át a Reaction modulnak.
	...
	\begin{itemize}
		...
		\item \textbf{Képbetöltési hibák:} Ha egy kép nem található, az alkalmazás fallback-megoldásként az item nevét jeleníti meg.
	...
	Az adatbázis kapcsolatokat illetve kulcsokat nem tartalmaz, a további feldolgozást a program végzi.
	...
	A játékban használt grafikai elemeket a Piskel\cite{Piskel} online szerkesztő segítségével készültek. A sprite-ok PNG formátumú, átlátszó hátterű képek, amelyek könnyedén felhasználhatóak a PyGame segítségével.
	
	Ilyen sprite a \ref{fig:Pipetta}. ábrán látható pipetta is, amely víz felvétele után már anyagként jelenik meg; ezt \ref{fig:Pipetta(H2O)}. ábra szemlélteti.
	...
	Az automatikus tesztek futtatása a \texttt{python -m pytest src/test.py} paranccsal történik. A tesztfutás eredményei a \texttt{test\_results.txt} fájlba kerülnek.
	...
	A rendszer jelenlegi korlátja a reakciókezelés implementációja. A \texttt{src/reaction.py} modulban alkalmazott \texttt{frozenset} alapú megközelítés egyszerű és hatékony az alap bináris reakciók kezelésére, azonban nem támogatja megfelelően az azonos reagenseket.
	...
	\begin{thebibliography}{2}
		...
		\bibitem{Gamifikacio}
		\textsc{Duchon Jenő}: \emph{Gamifikáció az oktatásban}, https://www.researchgate.net/\\publication/356749054\_Gamifikacio\_az\_oktatasban (Letöltés: 2026.04.22.)
		 
		\bibitem{Gamifikacio2}
		\textsc{Sharma, Wamika – Lim, Weng Marc – Kumar, Satish – Verma, Aastha – Kumra, Rajeev}: \emph{Game on! A state-of-the-art overview of doing business with gamification}, https://www.sciencedirect.com/science/article/pii/S004016252300673X (Letöltés: 2026.04.22.)
