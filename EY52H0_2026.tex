...
\begin{document}
	...
	A digitális oktatás fejlődése az elmúlt pár éveben egyre nagyobb hangsúlyt kapott, különösen az interaktív és vizuális tanulási módszerek alkalmazása terén. A hagyományos oktatási módszerek mellett egyre fontosabbá váltak azok a szoftveres megoldások, amelyek játékos módon támogatják a tanulók ismeretszerzését. Igaz ez a természettudományos tantárgyak esetében, ahol az elméleti tudást a vizuális megvalósítás is támogatja
	
	A kémia oktatásában a kísérletek és reakciók vizuális megfigyelése és kipróbálása kifejezetten fontos, hiszen ezek segítségével a tanulók könnyebben megérthetik az egyes anyagok tulajdonságait. Ugyanakkor az iskolai laboratóriumi környezet sok esetben korlátozott lehet. Akár az eszközök hiánya, a veszélyes anyagok használata, vagy az idő- és költségigény gyakran megnehezíti a gyakorlati oktatást.
	
	Általános iskolában illetve gimnáziumban a saját bőrömön is megismerhettem a labori környezetet amelyet főként csak a specifikus irányba továbbtanuló diákok használhattak többségben. Azonban ha kísérletezésre került sor az óra keretein belül, az jelentős felkészülést és időt vett el a tanórából. Ekkor értettem  meg igazán miért is az elméleti oktatásra fókuszálnak a pedagógusok. 
	
	Ezért is szakdolgozatom célja egy olyan oktatási célú játékalkalmazás létrehozása lett, amely virtuális laboratóriumi környezetben teszi lehetővé különböző kémiai reakciók szimulációját időtakarékosan. Az alkalmazás elsődleges célja, hogy támogassa a tanulók kémiai ismereteinek elmélyítését, valamint játékos formában segítse az összefüggések megértését. E mellett fontosnak tartottam azt is, a diákok akár maguk is próbálkozhassanak tanári segítség nélkül az órán tanult vegyületek készítésének tanulásával.
	
	Az programom lehetőséget biztosít különböző anyagok és eszközök kiválasztására, azok játékmezőn történő elhelyezésére, valamint az elemek közötti reakciók automatikus felismerésére. A vizuális megjelenítés és az interaktív használat elősegíti a tanulók memóriájának fejlesztését.
	
	Egyik fontos funkció a keresőrendszer, amely támogatja a felhasználót egy adott anyag vagy reakció létrehozásának folyamatában. Ez különösen hasznos lehet a tanulási folyamat során, mivel lépésről lépésre prezentálja a helyes megoldás sorrendjét.
	...
	A témaválasztásomat elsősorban az oktatás és az informatika összekapcsolásának lehetősége motiválta. Lelkes támogatóm volt édesanyám is az ötletben, akinek köszönhetem a szakmai tudását a kémiai reakciók iktatása során.
	...
	A kémia tantárgy sajátossága, hogy az elméleti ismeretek mellett fontos szerepet kap a gyakorlati megvalósítás, mivel a tudat alatti befogadóképessége számos embernél jobb, hogyha nem csak papíron lát egy adott reakciót, hanem saját kezűleg is képes megtapasztalni. A laboratóriumi kísérletek segítik a tanulókat abban, hogy jobban megértsék az anyagok tulajdonságait, valamint a különböző reakciók sorrendjét. Ugyanakkor a valós laboratóriumi környezet használata sok esetben korlátozott, mivel speciális eszközöket, anyagokat és biztonságos körülményeket igényel.
	
	Másrészt azért tartottam különösen fontosnak, mert egy virtuális laboratóriumi környezet lehetőséget biztosít arra, hogy a tanulók veszélytelen, könnyen hozzáférhető és költséghatékony módon gyakorolhassák a kémiai reakciókat. Ezen kívül a játékos megközelítés motiváló hatással lehet a diákokra, különösen a fiatalabb korosztály számára.
	...
	A Python név Monty Python-ról született meg, ugyanis Guido van Rossum egyik kedvenc humoristája volt.
	
	Manapság főként mesterséges intelligenciához, gépi tanuláshoz, játékfejlesztéshez használják
	
	Egyetemi éveim alatt több programozási nyelvel is megismerkedtem, ilyenek például a C++\cite{C++}, C-Sharp\cite{C-Sharp} vagy Java\cite{Java}, azonban fő programozási nyelvként a Python-t választottam, amellyel pályafutásomat kezdtem már gimnáziumban. 
	...
	\begin{itemize}
		...
		\item \textbf{Tágas könyvtárkészlet:} A Python közösségben számos könyvtár elérhető a különféle szükségekhez.
		\item \textbf{Keresztplatformosság:} A Python programok Windows, Linux és macOS rendszereken azonos módon futtathatóak.
		\item \textbf{Oktatási célokra alkalmas:} A Python széles körben használt oktatási nyelvként, amely ezáltal ideális egy hasonló célú alkalmazáshoz.
	\end{itemize}
	...
	A Structured Query Language (röviden SQL\cite{SQL}) az 1970-es években, IBM által elkezdett adatbázis kezelésre fejlesztett nyelv. Főbb lehetőségei a lekérdezés, módosítás, törlés illetve adatbázis létrehozása és kezelése.
	
	Szakdolgozatom során a már egyetemen megismert MySQL-el dolgoztam, amely lehetővé tette a gyors és egyszerű adatbázis létrehozást.
	...
	A fejlesztést lokálisan végeztem, ezáltal a Git számomra az a kód elérhetősége és letölthetősége során vált fontossá.
	...
	\begin{itemize}
		...
		\item Helyi backup-okat és az adatvédelemben if fontos szerep lát el.
	\end{itemize}
	
	A projekt elérhetővé tételéhez a GitHub platformot alkalmaztam, amellyel lehetőve vált:
	\begin{itemize}
	...
	A Visual Studio Code (röviden VS Code) egy a Microsoft által létrehozott ingyenes és nyílt forráskódú kód-szerkesztő. Első verziója 2015-ben jelent még, azóta egyik legismertebb fejlesztői eszközzé vált.
	...
	Ahogyan a Python, a VS Code is támogatja a crossplatformot, használható Windows, Linux és macOs rendszereken is.
	...
	Grafikus megjelenése miatt elősegíti az adatbázisok vizuális megjelenését és azok módosításait. Nem szükséges SQL alaptudás hozzá, manuálisan is kezelhető.
	...
	GitHub repository: \url{https://github.com/MRekaa/EY52H0_Szakdolgozat} 
	...
	A csomag használata lehetővé tette, hogy az alkalmazás adatvezérelt módon működjön, így az új anyagok és reakciók hozzáadása egyszerűen, az adatbázis bővítésével megvalósítható, a programkód  módosítása nélkül.
	...
	\begin{itemize}
		...
		\item Ha két item nem képes reakcióra nem történik változás.
	...
	Az adatbázis MySQL alapú, és legalább harmadik normálformában (3NF) van tervezve. A fő táblák:
	\begin{itemize}
	...
	A játék fő ciklusa: Eseménykezelés (egérkattintás, mozgatás), itemek frissítése, reakciók ellenőrzése, renderelés.
	...
	Az alkalmazás moduláris felépítésű, MVC-szerű (Model-View-Controller) architektúrával épül fel. Ez a megközelítés elválasztja az adatkezelést (Model), a felhasználói felületet (View), valamint az üzleti logikát (Controller), ezáltal biztosítva a kód szervezettségét, módosíthatóságát és karbantarthatóságát.
	
	A rendszer három fő rétegből áll:
	\begin{enumerate}
		\item \textbf{Adatkezelési réteg (Data Layer):} Az adatbázissal való kommunikációt és az adatok betöltését kezeli. Ez a réteg a MySQL adatbázissal kapcsolódik a \texttt{mysql-connector-python} könyvtáron keresztül.
		\item \textbf{Üzleti logikai réteg (Business Logic Layer):} Az alkalmazás magját képezi, ide tartozik a játéklogika, a kémiai reakciók ellenőrzése, az objektumok közötti interakciók kezelése.
		\item \textbf{Prezentációs réteg (Presentation Layer):} A Pygame keretrendszer felhasználásával valósul meg, kezeli a grafikus megjelenítést, a felhasználói interakciókat és a vizuális visszajelzéseket.
	\end{enumerate}
	...
	Az a játékmezőn jelenlévő objektumokat (anyagok és eszközök) reprezentálja. Ez az osztály a Model komponensként funkcionál az MVC architektúrában. Fő feladata:
	\begin{itemize}
	...
	\end{itemize}
	Az ItemOnBoard osztály közvetlenül kommunikál a Reaction modullal az ütközésdetekció eredményeit átadva, és a View réteg felé a Pygame-on keresztül a megjelenítéshez szükséges adatokat nyújtja.
	...
	Az objektumok törlésének funkcionáltatásáért felelős  trash osztály. Alapvető feladatai közé tartozik a törlési zóna megjelenítése és pozícionálása az ablak egy rögzített helyén (jobb felső sarkokban).
	...
	\subsubsection{Tool/materials osztály (tool.py/materials.py)}
	...
	\begin{itemize}
		...
		\item \textbf{Grafikus elemek és spritok:} A Pygame a \texttt{pygame.sprite} osztályokkal kezeli az objektumokat. Az alkalmazásban az ItemOnBoard osztályok Pygame spriteok-ból örökölnek, amely megkönnyíti az kombinációkat és a megjelenítést.
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item SQL lekérdezések végrehajtása: SELECT, INSERT, UPDATE, DELETE
	...
	\begin{figure}[h]
		...
		\begin{minipage}{0.45\textwidth}
			\centering
			\includegraphics[width=\textwidth]{img/tools/Pipetta.png}
		...
		\begin{minipage}{0.45\textwidth}
			\centering
			\includegraphics[width=\textwidth]{img/materials/Pipetta(H2O).png}
	...
	A program futtatásához a következő lépések szükségesek:
	\begin{itemize}
		\item Python 3.10 telepítése és virtuális környezet létrehozása a projekt mappájában.
		\item A szükséges csomagok telepítése a virtuális környezetbe: \texttt{pygame} és \texttt{mysql-connector-python}.
		\item MySQL szerver futtatása a helyi gépen, és a \texttt{labor} nevű adatbázis létrehozása.
		\item Az adatbázishoz tartozó táblák: \texttt{reactions}, \texttt{materials}, \texttt{}.
		\item A \texttt{src/config.py} modul beolvassa az adatokat az adatbázisból, így a táblák helyes struktúrája és tartalma szükséges a program működéséhez.
		\item Ha a \texttt{mysql-connector-python} csomag hiányzik, vagy a MySQL nem elérhető, a \texttt{src/dataBase.py} hibaüzenetet ad, és a betöltés üres lista lesz.
	\end{itemize}
	
	Az alkalmazás indítása előtt aktiválni kell a virtuális környezetet, majd a fő scriptet kell futtatni:
	\begin{itemize}
		\item PowerShell-ben a környezet aktiválása: \texttt{.\textbackslash venv\textbackslash Scripts\textbackslash Activate.ps1}
		\item A játék elindítása: \texttt{.\textbackslash venv\textbackslash Scripts\textbackslash python.exe src/main.py}
		\item Ha a virtuális környezet már aktív, elég közvetlenül futtatni: \texttt{python src/main.py}
	\end{itemize}
	
	Ha nincs \texttt{requirements.txt}, akkor a csomagokat kézzel kell telepíteni a fent említett módon.
	...
	\end{table}
	Az automatikus tesztek megvalósításához a Mockito \cite{Mockito} keretrendszert alkalmaztam. A Mockito egy népszerű Java-alapú mocking könyvtár, amely lehetővé teszi tesztkörnyezetben helyettesítő objektumok létrehozását tényleges függőségek helyett.
	
	A Mockito használatával a függőségek viselkedése előre definiálható, valamint ellenőrizhető, hogy egy adott metódus milyen paraméterekkel és hányszor került meghívásra. Ez növeli a tesztek megbízhatóságát és olvashatóságát.
	
	\chapter{Összegzés}
	
	A szakdolgozat célja egy Python alapú, grafikus felülettel rendelkező oktatási alkalmazás megvalósítása volt, amely a kémiai reakciók tanulását játékos és vizuális formában támogatja. A fejlesztés során egy olyan rendszer létrehozása került előtérbe, amely nem csupán bemutatja a reakciókat, hanem interaktív módon lehetőséget biztosít azok kipróbálására és megértésére is.
	
	A megvalósított alkalmazás több szempontból is sikeresen teljesítette az eredetileg kitűzött követelményeket. A Pygame alapú grafikus felület biztosítja a felhasználói interakciókat, míg a MySQL adatbázis háttér lehetővé teszi a reakciók és anyagok rugalmas kezelését. A moduláris, MVC-szerű architektúra hozzájárult a rendszer átláthatóságához és bővíthetőségéhez, amely különösen fontos egy oktatási célú szoftver esetében.
	
	A fejlesztés során kiemelt szerepet kapott a felhasználói élmény kialakítása. A drag-and-drop mechanizmus, a reszponzív felület, valamint a vizuális visszajelzések mind azt a célt szolgálták, hogy a tanulási folyamat minél intuitívabb és motiválóbb legyen. A tesztelési fázis során megfigyelhető volt, hogy a felhasználók gyorsan elsajátítják az alkalmazás használatát, és a vizuális elemek jelentősen segítik az összefüggések megértését.
	
	A projekt elkészítése során számos technikai és tervezési kihívással kellett szembenézni, például az adatbázis és a játéklogika szinkronizálásával, valamint a valós idejű interakciók hatékony kezelésével. Ezek megoldása hozzájárult a programozási és rendszertervezési ismereteim elmélyítéséhez.
	
	A rendszer jelenlegi formájában stabilan működik, azonban több irányban is továbbfejleszthető. Ilyen lehet például a hang- és vizuális effektek bevezetése, a reakciók animálása, valamint egy részletesebb oktatási modul kialakítása, amely lépésről lépésre vezeti végig a tanulókat a kémiai folyamatokon.
	
	Összességében elmondható, hogy a létrehozott alkalmazás nemcsak technikai szempontból sikeres, hanem pedagógiai értéket is képvisel. A projekt rávilágított arra, hogy az informatika és az oktatás hatékony kombinációja jelentős mértékben hozzájárulhat a tanulási folyamatok modernizálásához és élményszerűbbé tételéhez.
	...
	\begin{thebibliography}{2}
		...
		\bibitem{Mockito}
		\textsc{Mockito}: \emph{Hivatalos Mockito dokumentáció és felhasználói útmutató}, https://site.mockito.org/ (Letöltés: 2026.04.21.)
